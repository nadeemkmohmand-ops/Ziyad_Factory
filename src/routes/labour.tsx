...
const empty = {
  name: "", cnic: "", phone: "", job_role: "cutter", salary_type: "monthly",
  ...
  return (
    <div>
      <PageHeader title="Labour" urdu="مزدور"
        actions={<Button onClick={() => { setEdit(null); setForm(empty); setOpen(true); }} className="bg-primary text-primary-foreground"><Plus className="h-4 w-4" /> Add Worker</Button>}
      />
      <Card className="p-0 overflow-hidden">
        <Table className="marble-table">
          <TableHeader><TableRow>
            <TableHead>Name</TableHead><TableHead>Role</TableHead><TableHead>Type</TableHead>
            <TableHead className="text-right">Wage</TableHead><TableHead>Phone</TableHead>
            <TableHead>Active</TableHead><TableHead></TableHead>
          </TableRow></TableHeader>
          <TableBody>
            {rows.length === 0 ? <TableRow><TableCell colSpan={7} className="py-8 text-center text-muted-foreground">No workers.</TableCell></TableRow> :
              rows.map((r) => (
                <TableRow key={r.id}>
                  <TableCell><div className="font-medium">{r.name}</div><div className="text-xs text-muted-foreground">{r.cnic ?? "—"}</div></TableCell>
                  <TableCell><Badge variant="outline">{r.job_role}</Badge></TableCell>
                  ...
                  <TableCell className="text-right">
                    <Button size="sm" variant="ghost" onClick={() => { setEdit(r); setForm({ name: r.name, cnic: r.cnic ?? "", phone: r.phone ?? "", job_role: r.job_role ?? "cutter", salary_type: r.salary_type ?? "monthly", monthly_salary: String(r.monthly_salary ?? ""), daily_wage: String(r.daily_wage ?? ""), join_date: r.join_date ?? todayISO() }); setOpen(true); }}><Pencil className="h-3 w-3" /></Button>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>{edit ? "Edit" : "Add"} Worker</DialogTitle></DialogHeader>
...
// expose for unused-import safety
export const _f = fmtDate;
