...
function Sales() {
  ...
  return (
    <div>
      <PageHeader title="Sales Orders" urdu="سیلز" subtitle="آرڈر بنائیں، ادائیگی کریں، بل پرنٹ کریں — Orders, payments, invoices"
        actions={<Button onClick={() => setOpen(true)} className="bg-primary text-primary-foreground"><Plus className="h-4 w-4" /> نیا آرڈر / New Order</Button>}
      />
      ...
      <Card className="p-0 overflow-hidden">
        <Table className="marble-table">
          <TableHeader>
            <TableRow>
              <TableHead>رسید / Invoice</TableHead>
              <TableHead>تاریخ / Date</TableHead>
              <TableHead>گاہک / Customer</TableHead>
              <TableHead>ادائیگی / Payment</TableHead>
              <TableHead className="text-right">کل / Total</TableHead>
              <TableHead className="text-right">ادا / Paid</TableHead>
              <TableHead className="text-right">باقی / Pending</TableHead>
              <TableHead>حالت / Status</TableHead>
              <TableHead></TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-3xl">
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setOpen(false)}>منسوخ / Cancel</Button>
          ...
          {detail && (
            <div className="mt-4 space-y-4">
              ...
              <div ref={invoiceRef} className="bg-white text-black p-8 rounded">
                {/* Invoice Header */}
                <div className="flex justify-between border-b-2 border-yellow-500 pb-4 mb-4">
                  <div>
                    <div style={{ fontFamily: "'Noto Nastaliq Urdu', serif", fontSize: 30, color: "#c9a84c", direction: "rtl" }}>
                      {factory?.name_urdu ?? "المدینہ فیکٹری"}
                    </div>
                    <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 18, color: "#1a1a2e" }}>
                      {factory?.name_english ?? "Al-Madina Factory"}
                    </div>
                  ...
                  <div className="text-right">
                    <div className="text-2xl font-bold text-gray-800">رسید / INVOICE</div>
                    <div className="text-sm font-mono text-gray-600">{detail.order_number}</div>
                    <div className="text-xs text-gray-500">{fmtDate(detail.order_date)}</div>
                    <div className="mt-2 px-3 py-1 rounded text-xs font-bold uppercase tracking-wide"
                      style={{ background: detail.status === "delivered" ? "#d1fae5" : "#fef3c7", color: detail.status === "delivered" ? "#065f46" : "#92400e" }}>
                      {detail.status}
                    </div>
                  </div>
                </div>
                ...
                <div className="mb-4 bg-gray-50 rounded p-3">
                  <div className="text-xs text-gray-500 uppercase tracking-wide">گاہک / Bill To</div>
                  <div className="font-semibold text-gray-800 mt-1">{cName(detail.customer_id)}</div>
                </div>
                ...
                <table className="w-full text-sm mb-4">
                  <thead style={{ background: "#1a1a2e" }}>
                    <tr>
                      <th className="text-left p-2 text-yellow-400" style={{ fontFamily: "'Noto Nastaliq Urdu', serif" }}>ماربل کی قسم / Item</th>
                      <th className="text-right p-2 text-yellow-400">مقدار / Qty</th>
                      <th className="text-right p-2 text-yellow-400">قیمت / Price</th>
                      <th className="text-right p-2 text-yellow-400">کل / Total</th>
                    </tr>
                  </thead>
                  <tbody>
                    {items.map((it) => {
                      ...
                      return (
                        <tr key={it.id} className="border-b border-gray-200">
                          <td className="p-2">
                            <div style={{ fontFamily: "'Noto Nastaliq Urdu', serif", fontSize: 16, direction: "rtl" }}>{c?.name_urdu}</div>
                            <div className="text-xs text-gray-500">{c?.name_english}</div>
                          </td>
                          <td className="text-right p-2 text-gray-700">{it.quantity} {it.unit}</td>
                          <td className="text-right p-2 text-gray-700">{PKR(it.unit_price)}</td>
                          <td className="text-right p-2 font-semibold text-gray-800">{PKR(it.total_price)}</td>
                        </tr>
                      );
                    })}
                  </tbody>
                ...
                {/* Totals */}
                <div className="ml-auto w-56 text-sm space-y-1 border-t-2 border-gray-300 pt-3">
                  <div className="flex justify-between text-gray-600"><span>کل رقم / Subtotal</span><span>{PKR(detail.total_amount)}</span></div>
                  <div className="flex justify-between text-green-700 font-medium"><span>ادا شدہ / Paid</span><span>{PKR(detail.paid_amount)}</span></div>
                  <div className="flex justify-between font-bold text-lg pt-2 border-t-2 border-gray-800 text-gray-900">
                    <span>باقی / Balance</span><span>{PKR(detail.remaining_amount)}</span>
                  </div>
                </div>

                {/* Footer */}
                <div className="mt-10 pt-4 border-t border-gray-200 flex justify-between items-end">
                  <div>
                    <div className="text-xs text-gray-400">دستخط / Signature</div>
                    <div className="mt-6 border-t border-gray-400 w-32 text-xs text-center text-gray-500">زیاد خان</div>
                  </div>
                  <div className="text-center">
                    <div style={{ fontFamily: "'Noto Nastaliq Urdu', serif", fontSize: 14, color: "#c9a84c", direction: "rtl" }}>
                      شکریہ — کاروبار کا شکریہ
                    </div>
                    <div className="text-xs text-gray-400">Thank you for your business</div>
                    <div className="text-[10px] text-gray-300 mt-1">Al-Madina Factory · Ziyad Khan</div>
                  </div>
                </div>
              </div>

              <Card className="p-4">
                <div className="text-sm font-medium mb-2 font-urdu">ادائیگی کی تاریخ / Payment History</div>
      ...
      <Dialog open={payDlg} onOpenChange={setPayDlg}>
        <DialogContent>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setPayDlg(false)}>منسوخ / Cancel</Button>
            <Button onClick={addPayment} className="bg-primary text-primary-foreground">محفوظ کریں / Save</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
