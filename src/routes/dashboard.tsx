...
interface Kpis {
  ...
  rawRockTons: number; marbleValue: number;
}
...
function Dashboard() {
  ...
  const [quick, setQuick] = useState<QuickEntry>({type:"sale",amount:"",note:"",date:new Date().toISOString().slice(0,10)});
  ...
  const load = async () => {
    ...
    const [
      ...
      sixMonth,payTypes,catData,recentRows,
    ] = await Promise.all([
  ...
  const exportPDF = async () => {
    ...
    const name = `AlMadina_Dashboard_${new Date().toISOString().slice(0,10)}.pdf`;
    pdf.save(name);
...
`*المدینہ فیکٹری — Al-Madina Factory*
📅 ${now.toLocaleDateString("ur-PK")}

💰 اس ماہ کی سیلز: ${PKR(kpis.monthSales)}
✅ وصول شدہ: ${PKR(kpis.monthReceived)}
⏳ واجب الوصول: ${PKR(kpis.pendingReceivable)}
👷 مزدوروں کی تنخواہ: ${PKR(kpis.labourPayable)}
🪨 خام پتھر: ${num(kpis.rawRockTons)} ٹن
💎 تیار ماربل قیمت: ${PKR(kpis.marbleValue)}

_Ziyad Khan — Co-Founder_`
    );
  ...
  return (
    <div>
      <PageHeader
        title="Dashboard"
        urdu="ڈیش بورڈ"
        subtitle="المدینہ فیکٹری — فیکٹری آپریشنز کا مکمل جائزہ"
        actions={
          <div className="flex flex-wrap gap-2">
            ...
            <Button size="sm" onClick={shareWhatsApp} className="gap-1 bg-green-600 hover:bg-green-700 text-white">
      ...
      <div ref={dashRef}>
        ...
        <Card className="p-5 mt-6">
          ...
          {recent.length===0?(
            <div className="text-sm text-muted-foreground py-8 text-center font-urdu">ابھی کوئی آرڈر نہیں۔</div>
          ):(
      ...
      <Dialog open={quickOpen} onOpenChange={setQuickOpen}>
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="flex gap-2 mt-1 flex-wrap">
                {(["sale","expense","payment","attendance"] as const).map(t=>(
                  <button key={t} onClick={()=>setQuick({...quick,type:t})} className={`px-3 py-1.5 rounded-lg text-xs border transition-all font-urdu ${quick.type===t?"bg-primary text-primary-foreground border-primary":"border-border hover:border-primary/40"}`}>
            ...
            <div>
              <Label className="font-urdu">رقم (PKR) / Amount</Label>
              <Input type="number" value={quick.amount} onChange={e=>setQuick({...quick,amount:e.target.value})} placeholder="0"/>
            </div>
            <div>
              <Label className="font-urdu">تاریخ / Date</Label>
              <Input type="date" value={quick.date} onChange={e=>setQuick({...quick,date:e.target.value})}/>
            </div>
            <div>
              <Label className="font-urdu">نوٹ / Note</Label>
              <Input value={quick.note} onChange={e=>setQuick({...quick,note:e.target.value})} className="font-urdu" placeholder="اختیاری نوٹ"/>
            </div>
        ...
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>
              ...
              <div className="text-sm text-muted-foreground">Import Excel / CSV / JSON</div>
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground font-urdu" style={{direction:"rtl"}}>
              Excel (.xlsx), CSV، یا JSON فائل اپ لوڈ کریں۔ ڈیٹا خودکار ڈیش بورڈ میں ظاہر ہوگا۔
            </p>
            <div className="border-2 border-dashed border-primary/20 rounded-xl p-8 text-center hover:border-primary/40 transition-colors">
              ...
              <p className="text-sm text-muted-foreground font-urdu mb-3">فائل یہاں گھسیٹیں یا کلک کریں</p>
              ...
              <Input id="file-import" type="file" accept=".xlsx,.xls,.csv,.json" className="hidden" onChange={handleFileImport}/>
            ...
            {importData&&(
              <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-lg p-3 text-sm font-urdu text-emerald-400" style={{direction:"rtl"}}>
                ✅ {importData.length} ریکارڈ لوڈ ہوئے — ڈیش بورڈ میں ٹیبل ظاہر ہوگا
              </div>
            )}
