...
function Reports() {
  ...
  return (
    <div>
      <PageHeader title="Reports" urdu="رپورٹس" subtitle="تاریخ کے مطابق رپورٹس — Date-filtered reports, PDF export, WhatsApp"
        actions={<>
          <Button variant="outline" onClick={() => window.print()} className="gap-1 font-urdu"><Printer className="h-4 w-4" /> پرنٹ</Button>
          <Button variant="outline" onClick={() => exportPdf("report-area")} className="gap-1 font-urdu"><Download className="h-4 w-4" /> PDF محفوظ</Button>
          <Button className="gap-1 font-urdu bg-green-600 hover:bg-green-700 text-white" onClick={() => {
            const msg = encodeURIComponent(`*المدینہ فیکٹری — Al-Madina Factory*\n📊 رپورٹ / Report\n${new Date().toLocaleDateString("ur-PK")}\n\nزیاد خان — شریک بانی / Co-Founder`);
            window.open(`https://wa.me/?text=${msg}`, "_blank");
          }}><Share2 className="h-4 w-4" /> واٹس ایپ</Button>
        </>}
      />
      <Card className="p-4 mb-4 flex flex-wrap items-end gap-3">
        <div><Label className="font-urdu">سے / From</Label><Input type="date" value={from} onChange={(e) => setFrom(e.target.value)} /></div>
        <div><Label className="font-urdu">تک / To</Label><Input type="date" value={to} onChange={(e) => setTo(e.target.value)} /></div>
      </Card>

      <div id="report-area" className="space-y-4">
        <Tabs defaultValue="sales">
          <TabsList className="flex-wrap h-auto">
            <TabsTrigger value="sales">سیلز / Sales</TabsTrigger>
            <TabsTrigger value="finance">مالیات / Finance</TabsTrigger>
            <TabsTrigger value="inventory">اسٹاک / Inventory</TabsTrigger>
            <TabsTrigger value="labour">مزدور / Labour</TabsTrigger>
            <TabsTrigger value="lending">قرض / Lending</TabsTrigger>
            <TabsTrigger value="production">پیداوار / Production</TabsTrigger>
          </TabsList>
