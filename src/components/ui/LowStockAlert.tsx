  ...
  useEffect(() => {
    const load = async () => {
      const [raw, fin] = await Promise.all([
        ...
        supabase.from("finished_marble_inventory")
      ...
      if (totalRaw < 5) {
        alerts.push({ id: "raw-total", name: `خام پتھر / Raw Rock`, type: "raw", qty: totalRaw });
      }
  ...
  return (
    <Alert className="mb-4 border-amber-500/40 bg-amber-500/10 text-amber-200 flex items-start gap-3">
      ...
      <AlertDescription className="flex-1 text-sm">
        <span className="font-urdu text-amber-300 font-medium" style={{ direction: "rtl" }}>
          اسٹاک الرٹ:{" "}
        </span>
        {items.map((it, i) => (
          <span key={it.id}>
            ...
            >
              {it.name}
              {it.qty !== null && ` (${it.qty} ${it.type === "raw" ? "tons" : "units"})`}
            </Link>
          </span>
        ))}
        {" "}
