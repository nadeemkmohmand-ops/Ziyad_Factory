...
/**
 * Download Excel template for a specific table with column headers and sample data.
 * This is a standalone utility that can be used outside the ExcelImportExport component.
 */
export function downloadTableTemplate(tableName: TableName) {
  const wb = XLSX.utils.book_new();
  const ws = XLSX.utils.aoa_to_sheet([
    [`Template for: ${LABELS[tableName]}`],
    [`Table: ${tableName}`],
    [`Date: ${new Date().toISOString().slice(0, 10)}`],
    [],
    ["Column names are in the row below. Do NOT change column headers."],
    ["Fill your data starting from the next row."],
  ]);
  ws["!cols"] = [{ wch: 50 }];
...
/**
 * Import data from Excel file into a specific table
 */
  ...
  return new Promise((resolve) => {
    ...
    reader.onload = async (ev) => {
      try {
        ...
        if (rows.length === 0) {
          resolve({ ok: 0, failed: 0, errors: ["No data in file"] });
          return;
        }
          ...
          const processed = batch.map((row) => {
            ...
            Object.keys(r).forEach((k) => {
              if (r[k] !== undefined && r[k] !== "") {
                ...
                if (/^\d+(\.\d+)?$/.test(val)) {
                  (r as Record<string, unknown>)[k] = Number(val);
                }
              }
            ...
            delete r.id;
            delete r.created_at;
            if (transformRow) r = transformRow(r);
            return r;
          });

          const { error } = await supabase.from(tableName).insert(processed);
          if (error) {
            failed += batch.length;
        ...
        resolve({ ok, failed, errors });
      } catch (e) {
        resolve({ ok: 0, failed: 0, errors: [String(e)] });
      }
    };
